...
class ElectronApi
{
    ...
    init()
    {
        ...
        ipcMain.handle("syphonSubscribe", (event, serverDesc) =>
        {
            ...
            const success = appleBridge.initClient(serverDesc, (ioSurfaceBuffer, width, height) =>
            {
                try
                {
                    const imported = sharedTexture.importSharedTexture({
                        "textureInfo": {
                            ...
                            "handle": { "ioSurface": ioSurfaceBuffer }
                        }
                    });
                    
                    if (!imported)
                    {
                        console.error("[Main] importSharedTexture returned undefined!");
                        return;
                    }
                ...
                {
                    console.error("[Main] Error importing/sending shared texture:", e);
                }
            });
            
            currentClientActive = success;
            return success;
        });

        ipcMain.handle("syphonUnsubscribe", () =>
        {
            if (currentClientActive)
            {
                appleBridge.stopClient();
        ...
        let currentPublishActive = false;
        ipcMain.handle("syphonPublishStart", (event, serverName) =>
        {
        ...
        ipcMain.handle("syphonPublishStop", () =>
        {
            if (currentPublishActive)
            {
                appleBridge.stopServer();
        ...
        ipcMain.on("syphonPublishFrame", (event, pixelBuffer, width, height) =>
        {
            if (currentPublishActive)
            {
