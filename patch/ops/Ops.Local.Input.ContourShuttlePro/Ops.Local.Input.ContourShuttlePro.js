/**
 * Ops.Local.Input.ContourShuttlePro
 * Interfaces natively with Contour ShuttlePRO v2 controller.
 */
const
...
function handleIpcEvent(_event, jsonStr) {
    ...
    try {
        ...
        op.logWarn("[ContourShuttlePro] Event error: " + e.message);
    }
}
...
function start() {
    ...
    ipcRenderer.on("shuttleProEvent", handleIpcEvent);

    ipcRenderer.invoke("shuttleProStart")
        .then((success) => {
            if (success) {
                active = true;
                outRunning.set(true);
                ipcRenderer.invoke("shuttleProIsConnected").then(connected => {
...
function stop() {
    ...
    if (ipcRenderer) {
        ipcRenderer.removeListener("shuttleProEvent", handleIpcEvent);
        ipcRenderer.invoke("shuttleProStop").catch(() => {});
    }
}
