/**
 * Ops.Local.Vision.HumanHand
 * Detects 2D human hand joint poses and chirality (left/right) in a texture using native macOS Apple Vision,
 * outputting an array of hand joints (x, y, confidence) in real-time.
 */
const
...
render.onTriggered = () => {
    ...
    // Downsample target calculations (384px max dimension for fast hand tracking)
    ...
    // Send to main process via IPC to run native Apple Vision hand tracking
    ipcRenderer.invoke("syphonDetectHumanHand", inputBuffer, targetW, targetH, Number(inMinConfidence.get()) || 0.1)
        .then((hands) => {
            isProcessing = false;
        ...
        .catch((err) => {
            isProcessing = false;
            op.logWarn("[HumanHand] Hand Tracking failed: " + err.message);
        });
};
