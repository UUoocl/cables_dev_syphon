/**
 * Ops.Local.Vision.HumanPose3d
 * Detects 3D human body joint poses in a texture using native macOS Apple Vision,
 * outputting an array of 3D joint landmarks (x, y, z relative to camera) in real-time.
 */
const
...
render.onTriggered = () => {
    ...
    // Downsample calculations (384px max dimension for fast 3D estimation)
    ...
    // Send to main process via IPC to run native Apple Vision 3D body pose detection
    ipcRenderer.invoke("syphonDetectHumanPose3d", inputBuffer, targetW, targetH)
        .then((poses) => {
            isProcessing = false;
        ...
        .catch((err) => {
            isProcessing = false;
            op.logWarn("[HumanPose3d] 3D Pose Detection failed: " + err.message);
        });
};
